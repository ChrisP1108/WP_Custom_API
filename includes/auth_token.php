...
final class Auth_Token
{
     ...
     * @param string|int|null $user_id - The user ID from the token.
     * @param int $session_id - The session ID from the token.
     * 
     * @return Response_Handler The response of the token remove operation from the self::response() method.
     */

    public static function remove_token(string $token_name, string|int $user_id = 0, string|int $session_id = 0): Response_Handler
    {
        ...
        if (!$remove_cookie_result->ok) return self::response(false, 500, null, null, "Token cookie removal failed for token name of `" . $token_name . "`.");
        ...
        if (!$remove_refresh_cookie_result->ok) return self::response(false, 500, null, null, "Token refresh cookie removal failed for token name of `" . $token_name . "`.");
    ...
    public static function generate(string $token_name, int $user_id, int $expiration_at = Config::TOKEN_EXPIRATION): Response_Handler
    {
        ...
        $data = strval($user_id) . '|' . $expires_at . '|' . $issued_at . '|' . $nonce;
        ...
        if ($encrypted_data === false) return self::response(false, 500, $user_id, null, "Encryption failed while generating the auth token.");
        ...
        if (!wp_is_using_https() && Config::TOKEN_OVER_HTTPS_ONLY) return self::response(false, 500, $user_id, null, "Token of `" . $token_name . "` could not be stored as a cookie on the client, as the `TOKEN_OVER_HTTPS_ONLY` config variable is set to true and the server is not using HTTPS.");
    ...
    public static function validate(string $token_name, bool $validate_header_nonce = true, int $logout_time = 0): Response_Handler
    {
        ...
        if (!$token->ok) return self::response(false, 401, null, null, "No token with the name of `" . $token_name_prefix . "` was found.");

        // Get refresh cookie
        $refresh_cookie = Cookie::get($token_name_prefix . "_refresh");

        // Check if refresh token exists
        if (!$refresh_cookie->ok) return self::response(false, 401, null, null, "No refresh token with the name of `" . $token_name_prefix . "_refresh` was found.");
        
        // Split the token into encrypted data and HMAC and check that it is valid.
        $token_split = explode(".", $token->data['value'], 4);
        if(count($token_split) !== 4) return self::response(false, 401, null, null, "Inadequate data from existing token. May be invalid.");

        // Split token data into encrypted data and HMAC
        list($iv_base64, $encrypted_data_with_iv_base64, $received_hmac_base64, $received_session_id_base64) = $token_split;
        ...
        $received_session_id = base64_decode($received_session_id_base64, true);

        if ($iv === false || $encrypted_data_with_iv === false || $received_hmac === false || $received_session_id === false) {
            self::remove_token($token_name_prefix);
            return self::response(false, 401, null, null, "Invalid base64 token format.");
        }
        ...
        $session_data = Session::get($session_id, $token_name_prefix, $id);
        ...
        $updated_session_data = Session::update($session_id, $token_name_prefix, $id, $session_data['additionals'], $hashed_updated_refresh_cookie_value, $hashed_updated_header_nonce_value);
