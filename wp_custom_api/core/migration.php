...
use WP_Custom_API\Core\Error_Notice;
...
class Migration
{
     ...
     *      in the database for any in which the class constatn RUN_MIGRATION is set to true if it hasn't been created yet.
     * 
     * @return void
     * 
     * @since 1.0.0
    ...
    public static function init_all()
    {
        ...
        foreach ($models_classes_names as $model_class_name) {
            ...
            if ($model::RUN_MIGRATION ?? false && !Database::table_exists($model::TABLE_NAME)) {
                $table_creation_result = Database::create_table($class::TABLE_NAME, $class::TABLE_SCHEMA);

                if (!$table_creation_result['created']) {
                    Error_Notice::generate('Error creating table in database', 'The table name "'
